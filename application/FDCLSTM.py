...
class MaskedConv1D(Layer):
    """Masked 1D convolution with padding "same". Especially useful in short text.
 """
...
class Solver(object):   
    ...
    def load_model(self):
	    ...
	    # conv_1 
	    conv2 = Conv1D(filters=100, kernel_size=3,padding="same")(embedded_sequences) # before was 5
            batch_2 = BatchNormalization()(conv2)         
            act2 = Activation('relu')(batch_2)
	    flat2 = Flatten()(act2)

	    # conv_2
	    conv3 = Conv1D(filters=100, kernel_size=5, padding="same")(embedded_sequences)
	    batch_3 = BatchNormalization()(conv3)
            act3 = Activation('relu')(batch_3)
	    flat3 = Flatten()(act3)
		
	    # conv_3
	    conv4 = Conv1D(filters=100, kernel_size=8, padding="same")(embedded_sequences)
	    batch_4 = BatchNormalization()(conv4)
            act4 = Activation('relu')(batch_4)
	    flat4= Flatten()(act4)

	    # dense overlaping layers 
	    ...
	    ALL2 = concatenate([flat1, flat2,flat3, flat4])
	   
	
	    merged = concatenate([feats_dense, ALL2])
	    ...
	    output_layer = Dense(128, activation='relu')(rnn_layer) 
            ...
            feats_input = Input(shape=(self.feats_dim,))
            #feats_input = intermediate_from_a
            #Input(shape=(self.params['max_seq_len'],), dtype='int32')
